...
const char * const BITCOIN_CONF_FILENAME = "netsis.conf";
const char * const BITCOIN_PID_FILENAME = "netsisd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "netsisd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Netsis
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Netsis
    // Mac: ~/Library/Application Support/Netsis
    // Unix: ~/.netsis
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Netsis";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Netsis";
#else
    // Unix
    return pathRet / ".netsis";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (stream.good()) {
        ...
        // Create an empty netsis.conf if it does not excist
